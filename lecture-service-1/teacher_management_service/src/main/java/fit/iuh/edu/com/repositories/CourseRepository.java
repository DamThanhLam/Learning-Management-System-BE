...
//
    ...
    public Course getCourseByStudentIDAndCourseID(String userId, String courseId) {
        DynamoDbEnhancedClient enhancedClient = DynamoDbEnhancedClient.builder().dynamoDbClient(dynamoDBClient).build();
        DynamoDbTable<Course> dynamoDbTable = enhancedClient.table("Course", TableSchema.fromBean(Course.class));
        Map<String, AttributeValue> expressionAttributeValues = new HashMap<>();
        expressionAttributeValues.put(":userId", AttributeValues.stringValue(userId));
        QueryConditional queryConditional = QueryConditional.keyEqualTo(Key.builder().partitionValue(courseId).build());
        QueryEnhancedRequest enhancedRequest = QueryEnhancedRequest.builder()
                .queryConditional(queryConditional)
                .filterExpression(Expression.builder().expression("contains(studentsId, :userId)").mergeExpressionValues(expressionAttributeValues).build())
                .build();
        return dynamoDbTable.query(enhancedRequest).items().stream().findFirst().orElse(null);
    }
}

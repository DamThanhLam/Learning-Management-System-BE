...
public class BucketServiceImpl implements BucketServiceBL {
    ...
    public Map<String, String> putObjectToBucket(String bucketName, MultipartFile multipartFile, String ...path ) throws IOException {
        ...
        String key = imageFolder+fullPath+uniqueFileName;
        // Tạo một yêu cầu PUT object với bucketName và objectName
        PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                .bucket(bucketName)
                .key(key)
                .build();
        CompletableFuture<PutObjectResponse> responseS3  = s3AsyncClient().putObject(putObjectRequest, requestBody);
        responseS3.join();
        responseS3.whenComplete((resp, ex) -> {
            if (ex != null) {
        ...
        response.put("url", "https://" + bucketName + ".s3." + region + ".amazonaws.com/" + putObjectRequest.key());
        return response;
    }
